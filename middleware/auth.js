    ...
    try {
        ...
        if (decoded.creator_id) {
            creatorSchema.findOne({ token: token }, (err, result) => {
                if (err) {
                    return res.status(403).send({ "status": "Authentication Error", "code": "403", "ResponseCreated": TimeStamp() });
                }
                if (!result) {
                    return res.status(403).send({ "status": "Invalid Authentication", "code": "403", "ResponseCreated": TimeStamp() });
                }
        ...
        if (decoded.user_id) {
            userSchema.findOne({ token: token }, (err, result) => {
                if (err) return res.status(403).send({ "status": "Authentication Error", "code": "403", "ResponseCreated": TimeStamp() });
                if (!result) {
                    return res.status(403).send({ "status": "Invalid Authentication", "code": "403", "ResponseCreated": TimeStamp() });
                }
                if (decoded.user_id) {
